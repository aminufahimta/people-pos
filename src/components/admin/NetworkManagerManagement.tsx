...
interface NetworkManagerManagementProps {
  onUpdate?: () => void;
}

const NetworkManagerManagement = ({ onUpdate }: NetworkManagerManagementProps) => {
  ...
  const fetchManagers = async () => {
    const { data: profiles } = await supabase
      ...
      `)
      .eq("user_roles.role", "network_manager")
      .order("full_name");
  ...
  const handleUpdateManager = async () => {
    ...
    try {
      ...
      toast.success("Network Manager updated successfully");
      setEditingManager(null);
      fetchManagers();
      onUpdate?.();
    } catch (error: any) {
      toast.error(error.message || "Failed to update Network Manager");
    }
  };
  ...
  const handleCreateManager = async () => {
    ...
    try {
      ...
      if (error) throw new Error(error.message || "Failed to create Network Manager");

      toast.success("Network Manager created successfully");
      setNewManagerForm({
        email: "",
        password: "",
        full_name: "",
      ...
      toast.error(error.message || "Failed to create Network Manager");
    } finally {
  ...
  const handleDeleteManager = async (managerId: string) => {
    if (!confirm("Are you sure you want to delete this Network Manager?")) return;
    ...
    try {
      ...
      if (error) throw new Error(error.message || "Failed to delete Network Manager");

      toast.success("Network Manager deleted successfully");
      fetchManagers();
      onUpdate?.();
    } catch (error: any) {
      toast.error(error.message || "Failed to delete Network Manager");
    }
  };
  ...
  return (
    <Card className="shadow-[var(--shadow-elegant)]">
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Network Manager Management</CardTitle>
        <Dialog>
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Create New Network Manager</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <Button onClick={handleCreateManager} disabled={isCreatingManager} className="w-full">
                {isCreatingManager ? "Creating..." : "Create Network Manager"}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Dialog>
                        ...
                        <DialogContent className="max-w-md">
                          <DialogHeader>
                            <DialogTitle>Edit Network Manager</DialogTitle>
                          </DialogHeader>
                          <div className="space-y-4">
                            <div className="space-y-2">
...
export default NetworkManagerManagement;
