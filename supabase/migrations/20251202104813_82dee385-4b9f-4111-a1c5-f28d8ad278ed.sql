-- Create a function to check if a notification type is enabled for a role
CREATE OR REPLACE FUNCTION public.is_notification_enabled(p_role app_role, p_notification_type text)
RETURNS boolean
LANGUAGE sql
...
AS $$
  SELECT COALESCE(
    (SELECT enabled FROM notification_settings 
     WHERE role = p_role AND notification_type = p_notification_type),
    true  -- Default to true if setting doesn't exist
  )
$$;

-- Update notify_task_assignment to check notification settings
...
BEGIN
  IF NEW.assigned_to IS NOT NULL AND (TG_OP = 'INSERT' OR OLD.assigned_to IS DISTINCT FROM NEW.assigned_to) THEN
    ...
    -- Email network managers if enabled
...
-- Update notify_task_message to check notification settings
...
BEGIN
  ...
  IF assignee_email IS NOT NULL AND NEW.sender_id != task_record.assigned_to AND is_notification_enabled('employee'::app_role, 'task_messages') THEN
    recipients := array_append(recipients, assignee_email);
  END IF;
  
  IF creator_email IS NOT NULL AND NEW.sender_id != task_record.created_by AND is_notification_enabled('project_manager'::app_role, 'task_messages') THEN
  ...
  IF is_notification_enabled('super_admin'::app_role, 'task_messages') THEN
    FOR recipient_record IN SELECT * FROM get_super_admin_emails() LOOP
  ...
  -- Add network managers if enabled
  IF is_notification_enabled('network_manager'::app_role, 'task_messages') THEN
    FOR recipient_record IN SELECT * FROM get_network_manager_emails() LOOP
      IF recipient_record.email IS NOT NULL AND recipient_record.email != sender_email THEN
        IF NOT recipient_record.email = ANY(recipients) THEN
          recipients := array_append(recipients, recipient_record.email);
  ...
  -- Add sales users if enabled
  IF is_notification_enabled('sales'::app_role, 'task_messages') THEN
    FOR recipient_record IN SELECT * FROM get_sales_emails() LOOP
      IF recipient_record.email IS NOT NULL AND recipient_record.email != sender_email THEN
        IF NOT recipient_record.email = ANY(recipients) THEN
          recipients := array_append(recipients, recipient_record.email);
